...
3. Create Cloudflare A record pointing to Azure VM public IP:
   ...
   Name: @
   Type: A
   IPv4: public-ip (Azure VM public IP)
   TTL: Auto
   ```
...
ssh -i <path-to-key> azureuser@public-ip

# Or use PEM key
ssh -i Devops-Academy-VM_key.pem azureuser@public-ip
```
...
# OR http://public-ip:9090 (direct access)
...
pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh '''
                    ssh -i ~/.ssh/azure_key azureuser@public-ip << 'EOF'
